...
'''
ANT flux at steady state. This script generates the figure 4b in the paper
08.11.23
GCG
'''
...
fig = plt.figure(1)
#fig.subplots_adjust(right=0.98, left = 0.3, bottom =0.2, top = 0.95,wspace=0.4, hspace=0.5)
fig.subplots_adjust(right=0.9, left = 0.3, bottom =0.2, top = 0.95)

color = ['0.1','0.15','0.2','0.25','0.3','0.35','0.4','0.45','0.5','0.55','0.6','0.65','0.7','0.75','0.8','0.85','0.9','0.95','1.0']

for i,dmi in enumerate(np.arange(0.01,1, 1e-3)):
        cdo_i = dmi
        ctm_i =  6.5 #mM
        nr_do_i = 0.45*6.02*cdo_i*vims*1e20
        nr_to_i =0.05*6.02*ctm_i*vims*1e20

        ode.set( pars = {'ac':nr_do_i,
                        'bc':nr_to_i})


# # #                  # Initial condition
        tmp = ode.compute('pol%3i' % i).sample()    # or specify dt option to sample to sub-sample
        plt.plot(ctm_i/cdo_i, 1e3*k7*(no1_ant - tmp['l'][-1]- tmp['al'][-1]- tmp['bl'][-1]- tmp['la'][-1]- tmp['lb'][-1]- tmp['ala'][-1]- tmp['alap'][-1]- tmp['blbp'][-1]- tmp['blb'][-1]-tmp['bla'][-1])/(Na*vmito) - 1e3*k8*tmp['bla'][-1]/(Na*vmito) ,'.', c=color[0],markersize=1)
plt.ylabel(r'ADP$\rm _i$ - ATP$\rm _m$ flux (mM/s)',labelpad = 0.8)
plt.xlabel(r'ATP$\rm _i$/ADP$\rm _i$ ratio',labelpad =0.8)
#plt.ylim(0,0.76)
#plt.savefig('my_ant_flux_net_vs_atp_adp_ratioc_last.png',dpi = 600)
